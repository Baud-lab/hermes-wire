...
main <- function() {
  option_list <- list(
    ...
    make_option("--outdir",         type="character", default="assoc_species"),
  ...
  wanted_prefixes <- paste0(opt$trait_prefix, genera, "_")  # e.g. s__Prevotella_
  traits_all <- grep(paste0("^(", paste(wanted_prefixes, collapse="|"), ")"),
                     colnames(residuals), value=TRUE)
  ...
  if (!length(traits)) {
    stop(sprintf("No traits matched --traits='%s' after genus filtering. Example: %s",
  ...
  if (!all(lab == lab_map)) { warning("label_ids mismatch; using mapping from scan_ids"); lab <- lab_map }
  sel_char <- as.character(sel_int)
  
  ## Align matrices by (label → scan_id) order
  residuals  <- residuals[lab,,drop=FALSE]
  ...
  ## ---- Association ----
  cur_ids <- sel_char
  cur_lab <- as.character(int2label[cur_ids])
  if (anyNA(cur_lab)) stop("int2label mapping produced NA for some scan IDs; check lookup.")
  pd_full <- data.frame(sample.id = as.integer(cur_ids),
                        row.names = cur_ids, stringsAsFactors = FALSE)
  
  for (tr in traits) {
    y <- as.numeric(residuals[cur_lab, tr]); if (length(y) != length(cur_ids)) stop("y mismatch")
    pd <- pd_full; pd$y <- y
    
    outdir_trait <- file.path(opt$outdir, "by_trait")
    dir.create(outdir_trait, showWarnings = FALSE, recursive = TRUE)
    out_file <- file.path(outdir_trait, paste0(tr, "_assoc.tsv.gz"))
    ...
    for (chr in names(keep_idx_by_chr)) {
      ...
      if (inherits(nm, "try-error")) { LOG("[", tr, "] fitNullModel failed on chr ", chr); next }
      ...
      if (!inherits(a, "try-error") && !is.null(a) && nrow(a) > 0) {
        keep_cols <- intersect(
          c("variant.id","chr","pos","Score.pval","Score","Score.SE","Est","Est.SE"),
          names(a)
        )
        res_parts[[part]] <- as.data.frame(a)[, keep_cols, drop=FALSE]; part <- part + 1L
      }
      rm(nm, it, a); gc()
    }
  ...
  fwrite(data.table(ok=TRUE, n_traits=length(traits)),
         file.path(opt$outdir, "species_done.tsv"), sep="\t")
  LOG("DONE")
}
