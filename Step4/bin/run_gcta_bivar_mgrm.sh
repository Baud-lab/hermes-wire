...
usage(){ echo "Usage: $0 --mgrm mgrm_list.txt --pheno gcta.pheno --pairs pairs.tsv --threads N --out_prefix out__"; exit 1; }
MGRM= PHENO= PAIRS= THREADS=1 OUT=rg_bivar_mgrm__
while [[ $# -gt 0 ]]; do
  case "$1" in
