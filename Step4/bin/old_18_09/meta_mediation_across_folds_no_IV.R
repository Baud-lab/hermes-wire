...
opt <- parse_args(OptionParser(option_list = list(
  ...
  make_option("--steiger_min_prop", type="double", default=0.60, help="Min fraction of folds with Steiger OK (loose rule)"),
...
# Helper to safely extract numeric column or NA
...
# replace +/-Inf from min() when all-NA
for (nm in c("F_alpha_min","R2_alpha_min","F_total_min","R2_total_min")) {
  if (nm %in% names(META)) set(META, i=which(!is.finite(META[[nm]])), j=nm, value=NA_real_)
}

# Multiple testing (meta-level) by outcome
use_q <- tolower(opt$sig_metric) == "q"
pthr  <- opt$p_level; qthr <- opt$fdr_level
...
# Narrative conclusion (same logic as fold level, using meta calls)
...
# ---- Meta-level IV diagnostics / gating ----
...
META[, steiger_prop := ifelse(is.finite(steiger_prop), steiger_prop, NA_real_)]
META[, steiger_ok_loose := is.finite(steiger_prop) & (steiger_prop >= steiger_thr)]
META[, steiger_ok_strict := isTRUE(steiger_all)]

# Two composite instrument flags (choose one in your tables/figures)
...
col_exist <- intersect(col_order, names(META))
setcolorder(META, col_exist)
