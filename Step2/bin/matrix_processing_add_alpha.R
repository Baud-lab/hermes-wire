...
parser$add_argument("-a", "--alpha", type="character", help="File with alpha diversity data")
...
if (alpha != "") {
  print("Reading alpha diversity data")
  load(alpha)
} else {
  stop("No count data provided")
}
...
### Step 2: Add alpha values to the final count data ###
print("Step 2: Add alpha values to the final count data")

count_data_alpha=count_data_alpha[rownames(count_data),]
count_data=cbind(count_data,count_data_alpha)

# Save
save(count_data,
     file = 'count_data_with_alpha.RData')
